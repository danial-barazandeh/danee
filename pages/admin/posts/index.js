...
export default function Posts() {
    ...
    const src = `/api/posts?limit=20&&page=${pageIndex}`


    const updatePublishedData = (id, published) => {
        axios.patch(`${util.baseUrl}api/posts/${id}`, {
            published: published,
                            ...
                            <tr className='border-b-2 border-slate-200' id={index} key={index}>
                                ...
                                <td>
                                    <div className='w-16 h-10 md:w-24 md:h-16 lg:w-32 lg:h-24 rounded-lg overflow-hidden'>
                                        <Image width={1920/2} height={1080/2} alt={post.image.url} src={post.image.url.length > 1 ? post.image.url : `${util.baseUrl}images/not-found.jfif`} className="w-full h-full"></Image>
                                    </div>
                                </td>
                                <td>
                                    <div className='flex flex-col justify-center items-center'>
                                        <Link href={util.baseUrl + "admin/posts/" + post.id}>
                                            <a>
